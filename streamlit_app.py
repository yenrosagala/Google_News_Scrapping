...
# ==========================================
# GERBANG AUTENTIKASI: DENGAN INPUT PASSWORD MANUAL DI AWAL RUN
# ==========================================
def cek_autentikasi_manual():
    ...
    # Jika belum terautentikasi, paksa user memasukkan password database
    if not st.session_state["db_authenticated"]:
        ...
        with col_b:
            ...
            with st.card:
                ...
                # Input password murni kosong tanpa default
                input_pass = st.text_input("Kata Sandi Database / Aplikasi:", type="password", placeholder="Ketik password di sini...")
                ...
                if tombol_buka:
                    if input_pass.strip() == "":
                        ...
                        # Menyimpan password hanya di memori sesi aktif (RAM browser/server sementara)
                        ...
                        st.rerun()
        st.stop()  # Blokir sisa kode di bawah agar tidak berjalan sebelum diisi

# Jalankan proteksi di baris paling atas runtime aplikasi
cek_autentikasi_manual()

# ==========================================
# CONTEXT MANAGER DATABASE (ONLINE / OFFLINE FALLBACK)
# ==========================================
# Menggunakan password dari session_state untuk menyusun koneksi string
...
def dapatkan_koneksi_db():
    if IS_POSTGRES:
        import psycopg2
        # Menyisipkan password yang dimasukkan user secara dinamis ke dalam URL koneksi Postgres
        # Contoh template di secrets: postgresql://postgres:{PASSWORD}@db.xxxx.supabase.co:5432/postgres
        ...
        # Jika menggunakan SQLite lokal, password ini bisa digunakan sebagai validasi aplikasi internal tambahan
        # Serta memastikan skrip tetap berjalan meskipun tanpa server PostgreSQL eksternal
...
def inisialisasi_database():
    try:
        ...
        # Jika password salah atau gagal terhubung ke PostgreSQL online, reset status login
        st.session_state["db_authenticated"] = False
        st.session_state["saved_db_password"] = ""
...
# Verifikasi dan inisialisasi koneksi dengan password yang diinput user
...
with st.sidebar:
    ...
    st.markdown("---")
    # Tombol Keluar untuk menghapus ram memori password sepenuhnya
